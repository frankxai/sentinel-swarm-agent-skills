...
*   **Central Workspace**: `C:\Users\frank\starlight\repos\sentinel-swarm-cockpit\`
*   **Production GitHub**: `https://github.com/frankxai/sentinel-swarm-agent-skills`
*   **OneDrive Sync Base**: `C:\Users\frank\OneDrive\`
*   **Shared Memory Substrate**: Local ChromaDB (`localhost:8000`)
...
### A. The Persistent Memory Variable
Always ensure your local session has access to the shared ChromaDB server:
*   **Variable**: `CLAUDE_MEM_CHROMA_HOST=localhost`
*   **Heuristic**: Before running complex coding or writing tasks, query the local ChromaDB server on **port 8000** to pull historical session contexts and coordinate files.
...
```markdown
### YYYY-MM-DD HH:MM - [Agent Role] Session Completed
- **Session ID**: [Brief identifier]
- **Actions Executed**:
  - [Action 1]
  - [Action 2]
- **OneDrive Sync Status**: [Verified / Syncing]
- **GitHub State**: [Pushed to main / Uncommitted]
```
...
### The 4-Step Handoff:
1.  **Read task.md First**: Upon session boot, always view **`task.md`** at the repository root to see what the previous agent completed and what remains.
2.  **Mark Tasks Active**: When beginning an item, mark it in `task.md` as `[/]` (in progress) to signal to other agents that you are actively working on it.
3.  **Local Git Commits**: Commit work incrementally to local git. Keep commits clean and atomic.
4.  **OneDrive Attestation**: Before terminating a session, check local directory paths and confirm that OneDrive is actively syncing any newly generated HTMLs or documents.
...
*   **No Mass Deletions**: Do **NOT** run CLI scripts to mass-delete corporate directories. Let the enterprise reimaging handle data destruction safely.
*   **No DLP Bypasses**: Never write scripts to circumvent local firewall or registry policies. Operate within standard developer tools (Git, HTTPS, standard cloud sync).
*   **Zero Leakage**: Never exfiltrate, commit, or sync files containing corporate customer names, partner details, internal OCI credentials, or company-confidential strategies.

---

_You are now synchronized with the Sentinel swarm. Respect the design contracts in design.md and taste.md, keep files lean, and coordinate for excellence._
